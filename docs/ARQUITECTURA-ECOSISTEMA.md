...
| Suscripciones | *(sin repo propio — hoy vive dentro de `pagos-sorsabsa`)* | ⚠️ Sistema **conceptualmente independiente** de Pagos — "¿quién tiene acceso a qué?" es una pregunta distinta de "¿cómo se procesó este cobro?" — aunque hoy comparten repo, base y schema Postgres (`pagos.suscripciones`, junto a `pagos.pagos`/`pagos.comercios`/`pagos.referido_*`). Expone su propio contrato HTTP (`/api/entitlements`, `/api/crear-trial`, `/api/extender-suscripcion`) que ya es, de hecho, un servicio aparte — solo falta que la infraestructura lo refleje. Ver `AUDITORIA-PORTERO-SSO.md` 🔴-6: el gap real hoy es que ningún producto (ni agente24siete) se integra con este sistema como un contrato propio — o lo ignora del todo, o lo trata como tablas internas de pagos-sorsabsa. **09-ago-2026, decisión de Gina: agente24siete SÍ debe tener suscripción real** (ver 🔴-6 actualizado) — en construcción. |
| Referidos | *(sin repo propio — hoy vive dentro de `pagos-sorsabsa`)* | ✅ **Verificado independiente de verdad**, 09-ago-2026: `pagos.referido_codigos`/`referido_invitaciones` usan `producto`/`sujeto` como strings opacos, sin FK ni CHECK hacia ningún producto — el propio código lo dice explícito ("este servicio sigue sin saber qué es un condominio"). Un producto nuevo lo adopta sin tocar una línea de `pagos-sorsabsa`, solo pidiendo su `PAGOS_API_KEY_<PRODUCTO>` (mismo costo de entrada que cualquier servicio autenticado — no es acoplamiento). El único acoplamiento real no está en el sistema, está en su premio: `recompensa_dias` asume que todo producto mide valor en "días de suscripción". **Corregido 10-ago-2026 — "le funciona a CondoManager/DomusCRM/JustiRed" no estaba verificado, y no es cierto para dos de los tres:** el premio se acredita al "convertir" (primer pago real, `referidos-registrar.js`: *"este endpoint NO acredita recompensas: eso ocurre al CONVERTIR"*), y ese disparo necesita un evento real de pago para engancharse. **DomusCRM: sin disparador** — `referrals/route.ts` solo llama a `referidos-resumen`/`referidos-invitar`, nunca a la conversión; verificado con grep completo, no existe ninguna ruta de upgrade-a-pago en todo el repo (mismo hueco ya documentado: DomusCRM no tiene página propia de suscripción/pago, ver `PENDIENTES-ECOSISTEMA.md`). **agente24siete: sin disparador**, por una razón de negocio distinta — sus clientes los da de alta un admin, no hay registro público con `?ref=` (ver `AUDITORIA-PORTERO-SSO.md` 🔴-6). **JustiRed: su suscripción de abogados SÍ llega a `pagos.suscripciones`** (vía `metadata.suscripcion` en `pagos-sorsabsa/api/iniciar` + `confirmar.js`, mecanismo genérico) **pero con un bug real** — manda como `sujeto` un id de transacción nuevo en cada pago, no una identidad estable del abogado, así que cada renovación mensual crea una fila nueva en vez de extender la existente (detalle en `AUDITORIA-PORTERO-SSO.md` 🔴-6). **Corrección sobre lo escrito hace un momento en este mismo documento: ningún producto tiene el premio de referidos conectado de punta a punta — no es un hueco por producto, es que el paso de "convertir" (acreditar `recompensa_dias`) nunca se construyó en `pagos-sorsabsa`.** `ls pagos-sorsabsa/api` (filtrado a "referido"): solo existen `referidos-codigo.js`/`referidos-invitar.js`/`referidos-registrar.js`/`referidos-resumen.js` — ninguno acredita la recompensa, `referidos-registrar.js` lo admite en su propio comentario ("eso ocurre al CONVERTIR") sin que ese paso exista en ningún archivo. Detalle completo en `AUDITORIA-PORTERO-SSO.md` 🔴-6. |
| Créditos / Saldo | *(no existe — hoy 100% local en `agente24siete`)* | ❌ **NO transversal, es el hueco real.** Verificado en `agente24siete/lib/saldo.js`: tabla `movimientos_saldo` vive en la base propia de agente24siete, keyed por `cliente_id` (concepto local, no el patrón opaco `producto`/`sujeto` del resto), con lógica de negocio propia mezclada adentro (`markup_uso` leído de `clientes`/`planes`). Si mañana otro producto necesita créditos prepagados (JustiRed ya lo insinuó, `PENDIENTES-ECOSISTEMA.md` #16 — "créditos de IA"), hoy tocaría reinventar esto o migrar agente24siete después — exactamente el riesgo de "desoldar" que este documento existe para evitar. **Propuesto, no construido:** generalizar a `pagos.creditos`/`pagos.movimientos_saldo` en `pagos-sorsabsa`, mismo patrón `producto`/`sujeto` que Suscripciones — `markup_uso` y cualquier regla de negocio se quedan en agente24siete (mismo patrón que `crearBotonPago`: el producto calcula el monto, el servicio compartido solo mueve el número). |
| SSO | `auth-sorsabsa` | ⚠️ 16/16 verdes; 6 apps registradas (domuscrm, condomanager, agente24siete, justired, convertidor, `iot` — este último desde 08-ago-2026, ver PENDIENTES-ECOSISTEMA.md #14). **Dato de arquitectura fijado 10-ago-2026 (`AUDITORIA-PORTERO-SSO.md` 🔴-12):** el proveedor OIDC de `sorsabsa-identity` (`gyqgorgfstffbgazhbnb`) está registrado como cliente de confianza **solo en `verticales_sorsabsa`** (`twkuidnjwhopbjnrhnxp`, el proyecto de CondoManager) — CondoManager, DomusCRM y JustiRed lo heredan gratis por ser schemas del mismo proyecto. **Cualquier producto con un proyecto Supabase PROPIO y separado** (hoy solo agente24siete, `nwcqaginlnzjlkgwifas`) recibe, al final del login, una sesión emitida por `verticales_sorsabsa` — nunca por su propio proyecto. Su verificación de JWT (`SUPABASE_URL`/`SUPABASE_JWKS_URL`) debe apuntar a `verticales_sorsabsa`, no a sí mismo — confundir esto costó una sesión entera de bucles en agente24siete. Si el ecosistema suma otro producto con proyecto propio (ej. un futuro IOT con Supabase dedicado), aplica la misma regla. |
| Notificaciones | `notificaciones-sorsabsa` | ❓ |
| Geo | `geo-sorsabsa` (`@sorsabsa/geo`) | ✅ v0.1.0, Leaflet + OpenStreetMap, sin API key — ver §4-bis |
| Design system | `diseno-sorsabsa` (`@sorsabsa/ui`) | ✅ **La versión vigente es la de `diseno-sorsabsa/package.json` — no se copia acá porque una versión escrita a mano envejece** (esta fila decía `v0.1.36` cuando ya iba por `v0.1.60`). **Consolidación cerrada el 23-ago-2026:** ningún producto mantiene un sistema de componentes en paralelo — JustiRed pasó de 48 componentes propios a 7 (build de 39 s a 10 s) y CondoManager de 9 a 7, ambos con **cero duplicados** según el check de conformidad. Lo que cada producto conserva es cola de integración legítima, no duplicación: el `Button` de CondoManager renderiza `next/link` y su `EstadoBadge` traduce estados del negocio a tonos. Detalle en `AUDITORIA-CONDOMANAGER.md` 🔵-6 y `AUDITORIA-JUSTIRED.md`. **Pendiente conocido:** el design system exporta `Toast` pero no un modo de dispararlo, y por eso JustiRed conserva el de shadcn (`PENDIENTES-ECOSISTEMA.md` §29.3). |
